...
These invariants hold across `src/core/budgets` ↔ `budgets.md`:
...
7. **Composes with abort/timeout.** Because `signal` is a plain `AbortSignal`, a `Budget` folds into a combined bound with an [abort](aborts.md) and a [timeout](timeouts.md) via `AbortSignal.any([abort, timeout, budget])` — the combined signal fires on whichever trips first (the agent-loop pattern).
8. **Traceable identity.** `id` is a stable string for the handle's lifetime — caller-supplied via `options.id`, or a `crypto.randomUUID()` default that is unique across instances. `max` is the configured ceiling, exposed read-only. The abort reason on exhaustion is the platform default `AbortError` `DOMException`.
9. **Event-free (for now).** A pure functional primitive: no Emitter, no `EventMap`, no `on` hook. Observability is a separate deferred pass (the ROADMAP), so the surface above stays minimal.
10. **DOC ↔ SOURCE method bijection.** The `## Methods` table lists exactly `BudgetInterface`'s public methods — exhaustive, both directions — and `Budget` exposes the same public methods, no more (AGENTS §22).
...
```ts
import { createAbort, createTimeout, createTokenBudget } from '@src/core'

const abort = createAbort() // external cancel
const timeout = createTimeout({ ms: 60_000 }) // wall-clock deadline
